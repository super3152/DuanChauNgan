...
public class BLLHoaDon {
    ...
    public static void HienThiChiTietHoaDon(JTable tbl, int MaHD) {
        ...
        try {
            while (rs.next()) {
                ...
                obj[9] = ChuyenDoi.DinhDangTien(rs.getDouble("thue"));
                obj[10] = ChuyenDoi.DinhDangTien(rs.getDouble("thanhtien"));
    ...
    public static double DoHoaDonKhiQuetScan(JTable tb, String MaSP) {
        pnlbanhang.tblChiTietHoaDon.getColumn("Ảnh").setCellRenderer(new BLLSanPham.mytable());
        DefaultTableModel tableModel = (DefaultTableModel) tb.getModel();
        ResultSet rs = DAO.DAOSanPham.LaySanPhamScan(MaSP);
        ...
        try {
            while (rs.next()) {
                ...
                obj[7] = ChuyenDoi.DinhDangTien(rs.getDouble("giabanle"));
                double UuDai = 0;
                obj[8] = ChuyenDoi.DinhDangTien(UuDai);
                double thanhTien = (rs.getDouble("giabanle")) * Soluong;
    ...
    public static double SuaSanPhamVaoChiTietHoaDon(JTable tb, DTOSanPham sp, int SoLuong, double UuDai, double Thue) {
        pnlbanhang.tblChiTietHoaDon.getColumn("Ảnh").setCellRenderer(new BLLSanPham.mytable());
        DefaultTableModel tableModel = (DefaultTableModel) tb.getModel();
        Object obj[] = new Object[11];
        try {
            ...
            obj[9] = BLL.ChuyenDoi.DinhDangTien(Thue);
            double ThanhTien = sp.getGiaBanLe() * SoLuong - TinhChietKhau + Thue;
            obj[10] = BLL.ChuyenDoi.DinhDangTien(ThanhTien);
            JLabel lb = new JLabel();
        ...
        double tongTien = 0;
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            double tien = BLL.ChuyenDoi.ChuyenTien(tb.getValueAt(i, 10).toString());
            tongTien = tien;
        }
        return tongTien;
    }

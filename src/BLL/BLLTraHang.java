    ...
    public static int HienThiChiTietTraHang(JTable tbl, int MaTraHang) {
        ...
        try {
            while (rs.next()) {
                ...
                if (rsMSP.next()) {
                    obj[1] = rsMSP.getString("tensanpham");
                }
                obj[2] = rs.getInt("soluong");
                obj[3] = ChuyenDoi.DinhDangTien(rs.getDouble("giahang"));
                obj[4] = ChuyenDoi.DinhDangTien(rs.getDouble("phitra"));
                ...
                for (int i = 0; i < tbModel.getRowCount(); i++) {
                     soluong = soluong+ Integer.parseInt(tbl.getValueAt(i, 2).toString());
                    SoLuong =soluong;
                }
    ...
    public static double HienThiChiTietHoaDon(JTable tbl, int MaHoaDon) {
        ...
        try {
            while (rs.next()) {
                ...
                obj[5] = ChuyenDoi.DinhDangTien(GiaLe);
                double PhiTra = 0;
                obj[6] = ChuyenDoi.DinhDangTien(PhiTra);
                double ThanhTien = GiaLe * SoLuong - PhiTra;
                obj[7] = ChuyenDoi.DinhDangTien(ThanhTien);
                 ResultSet rsASP = DAO.DAOPhieuNhap.LayThongTinSPTheoID(MaSP);
                if (rsASP.next()) {
                ...
                tbModel.addRow(obj);
                for (int i = 0; i < tbModel.getRowCount(); i++) {
                    double tien = BLL.ChuyenDoi.ChuyenTien(tbl.getValueAt(i, 7).toString());
